					...
					<?php foreach (wc_get_account_orders_columns() as $column_id => $column_name) : ?>
						<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr($column_id); ?>" data-title="<?php echo esc_attr($column_name); ?>">
							...
							<?php elseif ('order-status' === $column_id) : ?>
								<?php $frontstatus = wc_get_order_status_name($order->get_status());

								echo esc_html($frontstatus);
								$dirct = plugin_dir_url(__FILE__);
								if ($frontstatus == 'ETH Payment Processing') {

									echo '<img src="' . $dirct . 'loader.png"  onClick="window.location.reload();" style="width: 40px;" id="loadimg">';
								}
